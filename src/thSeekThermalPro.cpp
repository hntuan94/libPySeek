...
thSeekThermalPro::thSeekThermalPro() :
    thSeekThermalPro(std::string())
{ }

thSeekThermalPro::thSeekThermalPro(std::string ffc_filename) :
...
bool thSeekThermalPro::init_cam()
{
    {
...
int thSeekThermalPro::frame_id()
{
    return m_raw_data[2];
}

int thSeekThermalPro::frame_counter()
{
    return m_raw_data[1];
}

bool thSeekThermalPro::seekInit()
{
	return open();
}

int thSeekThermalPro::seekRead(){
	if(!read(rawFrame))
	{
	...
	{
		cv::normalize(rawFrame, grayFrame, 0, 65535, cv::NORM_MINMAX);
		return 1;
	}
}

cv::Mat thSeekThermalPro::process(const cv::Mat& in) {
...
{
	return process(rawFrame);
}

cv::Mat thSeekThermalPro::seekGetGrayCvFrame()
{
	return process(grayFrame);
}
